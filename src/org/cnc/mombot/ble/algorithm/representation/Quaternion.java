package org.cnc.mombot.ble.algorithm.representation;

/**
 * The Quaternion class. A Quaternion is a four-dimensional vector that is used to represent rotations of a rigid body
 * in the 3D space. It is very similar to a rotation vector; it contains an angle, encoded into the w component and
 * three components to describe the rotation-axis (encoded into x, y, z).
 * 
 * <p>
...
public class Quaternion extends Vector4f {
	...
	/**
	 * Rotation matrix that contains the same rotation as the Quaternion in a 4x4 homogenised rotation matrix. Remember
	 * that for performance reasons, this matrix is only updated, when it is accessed and not on every change of the
	 * quaternion-values.
	 */
	 ...
	 * changed the quaternion numbers then the matrix will need to be updated. To save on processing we only really want
	 * to update the matrix when someone wants to fetch it, instead of whenever someone sets a quaternion value.
	 */
	 ...
	 * 
	 * @param input
	 *            The quaternion you want added to this quaternion
	 * @param output
	 *            The quaternion you want to store the output in.
	 */
	 ...
	 * 
	 * @param input
	 *            The quaternion you want subtracted from this quaternion
	 * @param output
	 *            The quaternion you want to store the output in.
	 */
	 ...
	 * @param rot
	 *            The angle of rotation around that vector in degrees.
	 */
	 ...
	 * 
	 * @param input
	 *            The quaternion to be slerped with this quaternion.
	 * @param output
	 *            The quaternion to store the result in.
	 * @param t
	 *            The ratio between the two quaternions where 0 <= t <= 1.0 . Increase value of t will bring rotation
	...
	public Quaternion inverse() {
		float norm = getX() * getX() + getY() * getY() + getZ() * getZ() + getW() * getW();
		if (norm > 0) {
			float invNorm = 1.0f / norm;
			setXYZW(-getX() * invNorm, -getY() * invNorm, -getZ() * invNorm, getW() * invNorm);
		}
		return this;
	}
